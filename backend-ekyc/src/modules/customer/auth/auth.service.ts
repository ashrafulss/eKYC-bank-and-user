...
export class AuthService {
  ...
  /**
   * 🌟 HIGHLIGHT: New isolated async notification engine
   * Runs inside the background worker ecosystem so SMTP network lag doesn't block the client response.
   */
    ...
    if (deliveryMethod === "email" || deliveryMethod === "both") {
      ...
      promises.push(
        this.transporter.sendMail(mailOptions)
          .then(() => console.log(`[SMTP Async] Verification email sent to: ${finalEmail}`))
          .catch((emailError: any) => console.error("SMTP background network error caught:", emailError))
      );
    }
    ...
    if (deliveryMethod === "sms" || deliveryMethod === "both") {
      promises.push(
        Promise.resolve().then(() => {
          console.log(`[SMS MOCK Async] Dispatch to ${mobile} -> Code: ${rawOtpCode}`);
        })
      );
    }

    // Process dispatches asynchronously in parallel background task chains
  ...
  async validateOTPVerification(mobile: string, otpCode: string, ipAddress?: string, userAgent?: string) {
    ...
    if (existingUser.current_step && !unverifiedStates.includes(existingUser.current_step)) {
